 ...
 * cannot explain.
 */
export default function RaceModeBanner({ circuits }: RaceModeBannerProps) {
  const { t } = useAppPref();
  ...
  return (
    <button
      type="button"
      onClick={openRandom}
      className="absolute bottom-4 left-1/2 z-30 flex -translate-x-1/2 items-center gap-3 rounded-md border border-red-500/25 bg-background/70 px-3 py-2 text-left shadow-2xl shadow-black/40 backdrop-blur-xl transition-colors hover:border-red-500/50 hover:bg-background/85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400/60"
    >
      ...
      <span className="leading-tight">
        <span className="flex items-center gap-2">
          <span className="text-sm font-semibold">{t.raceModeTitle}</span>
