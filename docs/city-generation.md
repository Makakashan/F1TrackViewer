...
| **D45** | The core belt builds its facade instead of printing it: a recessed glazing field held in mitred floor bands, solid corner piers, dark mullions standing proud, a ground-floor accent and a coping ring over a roof terrace with a pool. Every paint is measured off the reference. And ambient occlusion stopped occluding a wall with the wall it belongs to. | The tile could not get there, arithmetically. It multiplies the wall's own paint, so a navy window on a terracotta wall is unreachable and a band lighter than its wall doubly so, and both are what the reference is made of. As geometry they carry their own paint per vertex on a mesh that already exists, so the core lost its five facade materials and went from 21 draw calls to 11. The paints came from quantising the reference facade and point-sampling it, not from judgement: glazing #21396D at saturation 0.70 over 41 % of the facade, band face #F7E5D2 against a cooler #E9E1D8 soffit on the same slab, coping #FEFCF9, pier #E3BD91, and mullion #343A38 — the quantiser reported #88766A for the mullion and was wrong by a factor of two in value, because at 5 % of the facade the post never survives as its own cluster. The occlusion fix is the larger half. Measured on a free-standing block on empty ground, every wall vertex below the top storey came back at the floor, 0.450: the occluder grid stamps a building's height into its own cells, so a wall read itself in every direction that pointed inwards. Occlusion is now cosine-weighted over the hemisphere a surface faces and samples from a cell clear of its own solid; an upward face, which has no outward normal to step along, walks down the height gradient instead, and a downward face is left alone because a soffit really does face the ground. Wall faces 0.450 to 0.758, shelf tops 0.460 to 0.629, soffits held at 0.530 — the ordering that reads as depth. That single number is most of why the city read grey through four passes of palette work. A relation's footprint arrives as one ring with its parts strung together, so two of its edges are chords across the building; both ends clear the track and the middle does not. The tiled wall drew such a chord as one quad with no vertex between its ends and nothing measured it, while the frame puts a pier and a mullion along it — 620 vertices at 1.35 m from the centreline. The frame now samples every edge along its length and skips one that crosses the corridor, and where a wall stands exactly on the corridor edge its members go flush rather than the wall being dropped. Splitting multipolygon rings properly is still owed. Reviewed on screen and corrected: the belt is one mitred ring round the footprint rather than four slabs trimmed to their own projection, which only meets at a right angle and left ends hanging past every other corner; the band's shelf and soffit reach back to the glass rather than to the wall plane, which had left a slot at every floor to see straight through the building; the roof lid leaves a hole for the pool instead of the pool lying two centimetres over it, since the depth buffer resolves about three centimetres at 1.5 km and the roofs flickered; the projection and the recess came down from 0.42 and 0.5 m to 0.3 and 0.2 m, because from above nearly a metre of ledge a storey covered the glass; and the frame took its own 2.15 m bay off the reference's rhythm instead of the painted tile's 3.4 m. Looked at point blank, the band's shelf and soffit turned out to be wound the wrong way round — the shelf facing down and the soffit facing up — so both were culled and the eye went in over every band and out through the far wall. A render of the same view with every material double-sided, differenced against the single-sided one, is what found it and what says it is gone: what is left differs by antialiasing seams a pixel wide. The cornice went with it: the coping ring does that job now, and the two were drawn one over the other. Two more followed, both only visible on a curved plan: every reveal — the return between a pier or a mullion and the recess behind it — faced into the solid rather than into the recess, so on a straight wall the pier in front hid it and on a curve it left a vertical slot at each one, the slits the track showed through; and the coping reached back past the ring to the glass plane, leaving a slot all the way round the parapet, where a floor band has a recess to close and a coping has none. The check is now a mode of the harness rather than a one-off: `env:one --holes` renders each view twice, once with every material double-sided, and counts the pixels where the plain render shows sky and the double-sided one shows the model. Comparing the two frames outright cries wolf — over a parapet the inside of the far wall legitimately appears only when back faces are drawn — so only see-through counts. Block, tower, house, retail and plain, four to fourteen sides, four viewpoints each: watertight everywhere at or above ground level. Below it a building has no underside, which is true of every extrusion here and is never in shot. Core 293,712 triangles of 450,000 and 4.28 MB, city 215,812 and 4.58 MB, far 89,572 and 1.55 MB, 10.41 MB of 15; draw calls 11/19/5 of 75. |
| **D44** | A core-belt building gets a projecting cornice where its wall stops, a tower's top storeys step back 1.6 m onto a capped shoulder, the facade tile keeps a solid pier down one side of every bay, and the wall palette moved lighter. | Four complaints about the same flatness. An extrusion ends at a line, and what says "top" on a real street is the 0.5 m shelf that overhangs it — two faces an edge, core belt only, because at city range it is under a pixel. A tower that is one prism from pavement to sky is a slab: the crown steps in by 1.6 m over min(6 m, 18 % of the height) and stands on a `capRing` shoulder, so the silhouette breaks where a real block breaks. The pier is what makes a facade read as a solid with holes cut in it rather than a grid — openings now share `CELL - 0.16·CELL`, and the balcony slab starts after it. And the palette had to go lighter because the occlusion pass takes about a third of the value off inside a street canyon: the paints that measured "cream" on the swatch measured "grey" on the wall. Core 296,907 triangles of 450,000 and 3.89 MB, city 215,812 and 4.55 MB, far 89,572 and 1.55 MB, 9.99 MB of 15; draw calls 21/19/5 of 75. |
| **D43** | Openings are one light of 0.4–0.8 of a 3.4 m bay rather than two of 0.36 of a 2.7 m one, the glass is a cool blue rather than grey, and a roof is its own material — slate, not the wall seen from above. | Two small windows to a bay is a 0.97 m opening, and a city of them reads as a rash of dots from any distance a whole building fits the frame; one light of 0.7 on a wider bay is 2.4 m, a window somebody could stand at. The glass carries a little of the sky it reflects, which is what tells it from a shadow at a glance. The roof had to become a material rather than a darker vertex colour, because vertex colour is floored at 0.62 by I11 and a roof wants to be darker than that; as a material it is one more draw call a belt (city 19, limit 75) and the audit welds it back to the walls before asking what stands on what. City belt 207,539 triangles of 350,000, 4.34 MB of 7. |
| **D42** | A pitched roof is wound outwards, and no building over 12 m gets one — not even where the survey tags a shape. | Every face of every pitch was inward: measured, 6 of 6 on a gable and 4 of 4 on a pyramid, both windings of the ring. Culled, they showed the inside of the far slope, which is the "buildings with no roof, and only one wall from an angle" the roadmap had been blaming on the old runtime path since P4.4 was written. It was here all along. The ceiling now holds against the tag as well: `roof:shape` is right about the shape and wrong about whether a pitch is worth building on a ten-storey block, and Monaco tags plenty of them. 12 m rather than 16: a Monaco block of four storeys is flat-roofed, a villa on the hill is not. Roofs go 1,660 flat / 2,438 hipped to 2,072 flat / 2,127 hipped. |
| **D40** | Balconies run as a band across every floor or as stacks in the same bays all the way up, chosen per building, with rails and returns; and the tile stops painting one where the belt builds it. Walls take a colour from a six-paint palette, weathered a few per cent per building, multiplied verbatim rather than through `applyAlbedo`. | A roll per balcony scattered them over the wall, which no building does — a block either has a balcony every floor or a column of them, and what makes it read is that the column lines up. The tile also had to stop painting a slab under a built one: the same balcony twice, once flat. Colour is the other half of the reference the user brought — a Monaco street is cream and sand and terracotta, and the diorama was reading as one grey model because every wall was the same white. The paint goes on verbatim: `applyAlbedo` keeps only 35 % of the chroma, which is right for taming a kit's palette and turned these back into the grey they were picked to break. Each channel is floored at 0.62 rather than the tone alone, because a sand wall is already 0.85 on blue and a shop front's 0.65 under it lands beneath I11. Core belt 276,321 triangles of 450,000. |
| **D39** | A facade tile holds four cells rather than one, each with its openings a little different; every building starts reading the tile at its own offset and is painted a few per cent off its neighbour. Blocks in the core belt carry balconies as geometry. | One cell repeated is a grid of identical windows on every wall of every building, which is what "monotonous" means here — four cells cost nothing extra (the tile is 192×192 instead of 96×96) and read as a facade that was built rather than printed. The per-building offset is what stops the street starting on the same cell, and the tone jitter is what a row of houses looks like at a glance; both come from a hash of the plot, so the same bake gives the same city. Balconies are painted everywhere and built where the camera goes: a plate, its front and its underside, anchored 0.3 m into the wall — butted against it, seven of them welded as their own pieces and `env:audit` read them as floating. They are refused where they would reach into the corridor (242 vertices did) or inward on a re-entrant corner. Core belt 198,327 triangles of 450,000; the jitter also needed a floor, since 0.65 × 0.94 × the occlusion floor lands under I11's 0.278. |
| **D38** | Every building is classified — tower, block, house, retail, plain — and its walls are textured with a tile of that kind: one bay across, one storey up, repeating. Two tiles per kind, because a shop front is not a storey repeated. | Painted storey bands said "floors" and nothing about windows, and a facade is a pattern of openings. Geometry could not carry it: a window is two triangles and the city belt alone would have wanted about 700,000 of them, against 350,000 for the whole belt. A tile costs none — the wall is one quad however tall it is, because the tile repeats up it as well as across, which also took the walls from six vertices a storey to twelve for the building. Bytes: coordinates are quantised to 12 bits and the far belt has none at all, since it is silhouettes; city belt 3.94 MB of 7 and 202,191 triangles of 350,000, draw calls 42 of 75. The classifier is measured rather than surveyed: 1,676 of Monaco's footprints are tagged `building=yes` and 36 carry a height, so height and area decide and the tag only overrides — 317 towers, 1,535 blocks, 2,546 houses, 369 retail, 45 plain. `env:audit` welds the meshes back together before asking what stands on what: the split is a material split, and a roof on its own reads as a slab with no walls under it. |
