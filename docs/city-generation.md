...
| **D30** | The flat surfaces are drawn with `polygonOffset` rather than a bigger lift, and a park stands on a 0.75 m rim with a per-triangle shade over its planting. | Measured at the wide shot: without the offset the terrain ate **11.1 %** of the park area and flickered over the rest, because 0.12 m of lift is the depth buffer's whole precision at two kilometres with near at 2 m; with it, 0 %. Lifting the geometry instead would float a park at street level. The rim and the dapple answer the other half of the complaint — flat, a park read as green paint on grey, which is what killed the ground tint. The wall gives the shape a side the light finds, and ±14 % per triangle stops a hectare reading as one poured colour. City belt 214,682 → 215,078 triangles. |
