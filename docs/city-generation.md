...
| **D41** | The key light casts, over a 2.5 km box at 2048 — a wall's shadow, not a windowsill's. Walls, models, props, barriers and the plinth cast; ground, decks and lids take. The far belt takes but does not cast. | A diorama with no shadow reads as paper: nothing tells a block from a painted rectangle, which is what the reference model has and this did not. The box is fixed rather than fitted per frame because the city is centred on the origin and 3 km across; at 2048 that is 1.2 m to a texel, which draws the shadow of a building and not of its balcony. Cost, measured in the app over 40 renders with a read-back forcing the GPU to finish: 1.32× the frame with the sun off — and that ratio understates it, because the read-back adds the same stall to both sides. So it sits behind the quality lever with the rest: off on the performance path and on mobile. The far belt not casting is worth the last 0.04 of that ratio and costs nothing to look at, since a block at 600 m throws a few pixels. |
