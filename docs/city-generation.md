...
| **D37** | A building is a plain extrusion with a flat roof and a rim. The kit of modelled houses is switched off, the boxes on the roofs are gone, and the pitch generator — `roofs.ts` — is deleted. This reverses the whole 2–11 September detail push, which was abandoned at `c51e524` and restarted from there. | Nine days went into making a wall look like a wall: a facade tile of windows per building class, painted balconies, a six-paint wall palette, a built facade for the core belt with floor bands, corner piers, dark mullions and a recessed navy glazing field, a pool sunk into every large roof terrace, street ribbons laid on the ground, and a map palette of white blocks with sand shopfronts and a handful of coloured landmarks. Each pass fixed what the last one was blamed for and the answer stayed no: the city read as coloured cardboard, then as a punched card, then as a white city that was still a heap of components. The reference — the Bump app's Monaco over Apple's map — has no windows, no roof furniture and no pitched roofs on it at all, which is the thing nine days of detail work was arguing with. So the branch was cut at the last commit before the facades started and three things came off what was left. The kit: a model is somebody else's house stretched onto a Monaco plot on all three axes, and 173 of them among 4,572 extrusions read as a different, darker, pitched city dropped into this one. It is switched off by emptying the pack list, not by deleting the pass, so turning it back on is one array. The roof clutter: lift heads and vent stacks on every roof over 80 m² said "used" up close and "scattered dark cubes" from every distance the diorama is actually looked at. The pitches: `planRoof` fitted an oriented box round the footprint and put a ridge along its long axis, which is right for a rectangle and wrong for everything else — the ridge crossed the wrong way and the eaves hung over the street, and point blank a house read as a dark lid resting on a lighter box. `roof:shape` and `roof:height` are no longer read; `PARAPET_M` is all that survives of the module. What is left is the landscape, the coast and the water, every building in the bbox at its lidar height, the tunnels and their mouths, the plinth, the parks, the trees and the berthed yachts. Monaco goes from 10.12 MB to **6.34 MB** of 15 — core 1.52 MB / 143,103 triangles, city 3.48 MB / 200,701, far 1.34 MB / 77,754, draw calls 10/8/4 of 75. The city belt grew: a pitch stopped its walls at the eaves and a flat roof carries them to the top plus a rim. Nothing of the abandoned work is lost — it is on `feat/streets-on-the-ground`, on `dev`, and in one stash — and the street ribbons in particular stand on their own, since they never depended on a facade. |
| **D36** | Every extruded building gets a shop-front band and, on a flat roof, a lift head or two. A mesh carries a `tone` that the triangle adder writes into `albedo`, rather than a caller counting vertices. | The models fixed a few hundred buildings; the other four thousand were prisms. Two things say "building" at street distance and neither needs an asset: the line where the ground floor ends, and something standing on the roof. The band is paint on two extra rows of vertices, the clutter is boxes — at this size a lift head is a box in life too, and it is ten triangles rather than a thousand. Cost to the city belt: 32,394 triangles for both, paid for by dropping the kit's budget share from 0.5 to 0.42 (266 modelled buildings → 173). A band on all 4,572 beats a hundred more models on a few. Two bands rather than three: the parapet already draws the top edge. The tone is 0.65 because I11 floors vertex colour at 0.278 and the occlusion floor is 0.45 — a band cannot take more than a third of a wall. The `tone` field is the other half: a parallel array counted by the caller cannot survive `addFlatTriangle` dropping a degenerate one, and it did not — core was 5,789 vertices out of step, which painted the bands onto other people's walls. |
