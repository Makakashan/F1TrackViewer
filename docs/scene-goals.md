...
| **I3** | No belt seam opens. Where two belts overlap in plan the terrain is continuous: no gap the sky shows through, no overlap that z-fights. | `env:audit` over the overlap band; `terrain.test.ts` on a synthetic slope crossing a seam | **failing** — 583 of 1,136 shared points apart, mean 0.41 m, worst 11.68 m. Roadmap step 6 |
